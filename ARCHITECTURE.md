...
- `Packages/com.texturelab.editor/Editor/Palettes/PaletteExtractor.cs` — deterministic Oklab K-Means extraction from a GPU-downsampled 128px source; it does not require readable source imports. Palette Quantization cards create, assign, edit, reorder, and extract palette assets with Undo support. `Editor/Palettes/Standard/` contains 20 selectable built-in thematic `TextureLabPalette` assets shipped with the package; user palettes remain under `Assets`.
